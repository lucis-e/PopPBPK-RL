...
def euclid_func(state): 
    # Función inversa de la distancia planar cuadrática
    AUC_v = ((state[3] - TARGET1) / np.abs(TARGET1 - UB1))**2   # AUC is in position 3 of state vectors
    Cmax_v = ((state[4] - TARGET2) / np.abs(TARGET2 - UB2))**2     # Cmax is in position 4 of state vectors
    reward = 10 - (AUC_v + Cmax_v)  # TARGET2 = 175 (Cmax)  
    return(reward)  
